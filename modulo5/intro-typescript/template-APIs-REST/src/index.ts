...
const operacoes = (numero1: number, numero2: number):void =>  {

    const maior = numero1 > numero2 ? numero1 : numero2;
    const menor = numero1 > numero2 ? numero2 : numero1; 

    console.log(`A soma entre eles resultou em: ${numero1 + numero2}`);
    console.log(`A subtração entre eles resultou em: ${Math.abs(numero1 - numero2)}`);
    console.log(`O ${maior} é maior que o número ${menor}`);
}
...
const transformarDna = (dna: string) => {
   let rna = dna.replace(/A/g, 'U').replace(/T/g, 'A').replace(/C/g, 'g').replace(/G/g, 'C')
...
const reverter = (valor: string) => {
    let dado = valor.split('');
    dado.reverse();
    console.log(`O valor invertido é ${dado.join('')}`)
}
...
const estudanteLabenu = (idade: number, ensino: boolean, disponibilidade: number, horario: string) => {
    if (idade < 18) {
